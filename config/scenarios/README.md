# `config/scenarios/` — seeded missions the runner and the gate consume
...
```bash
./runtime/local/run_scenario.py config/scenarios/square-10m.yaml --seed 3 --outdir out   # one run, one JSON result + MCAP
./runtime/local/run_gate.py     config/scenarios/square-10m.yaml --outdir out   # 10 seeded runs, SR must be 100%
```

Both drive [`../runtime/local/sim_up.sh`](../runtime/local/sim_up.sh) — the simulator's only supported
