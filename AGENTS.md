...
up as containers by `./runtime/local/sim_up.sh`. Bring your own Unreal world (`.uproject`), place
...
spec), `docs/todo.md` (the backlog) and `third_party/versions.lock` (every pin). The historical design
...
- **A stale PX4 EKF origin looks exactly like a control bug.** PX4 sets its EKF local
  ...
  a full day. `runtime/local/sim_up.sh` verifies the origin and repairs it by restarting PX4;
  `runtime/local/run_gate.py` scores an unrepairable run **VOID, not FAIL**, because such a run
  never measured the flight code at all.
- **Frames are NWU, not ENU**, despite what upstream documentation says.
  `ros2/src/control/control/frames.py` carries a tested conversion.
...
- **Sensor rates are capped in `perception.launch.py`** — imagery 20 Hz, LiDAR 10 Hz — and
  ...
  (`runtime/local/measure_sensor_rates.sh`). A rate below the ceiling is a finding; the ceiling
...
- **There is no compose file.** The simulator has never used `docker compose`; containers
  come up through `./runtime/local/sim_up.sh` with raw `docker run`. `runtime/local/check_image_refs.py`
  is the tier-1 CI check that keeps image names honest — every `drone-sim/...` reference in
  the tree must name an image declared under `images:` in `third_party/versions.lock`.
...
- **Feature work / code changes → branch and open a PR.** Anything touching the ROS 2
  ...
  Dockerfiles, or `third_party/versions.lock` — **especially large changes** — goes on a feature
...
- **Documentation-only changes → direct to `main` is fine.** Edits confined to
  docs, worklogs, READMEs, and `AGENTS.md` guidance may be committed and pushed
...
- **Submodules / vendored trees.** This project pins third-party code (PX4, `px4_msgs`,
  Cosys-AirSim, the Micro-XRCE-DDS Agent) — prefer a `vcstool` `third_party/sources.repos` manifest over git
...
- **Documentation** — Markdown, worklogs, READMEs, `AGENTS.md` guidance, design docs,
  changelogs.
...
the only real client. **Bring the stack up with `./runtime/local/sim_up.sh`, exercise the *full*
...
- **Flight / control / perception behaviour → fly it and capture the evidence.**
  - Run a **seeded scenario** (`runtime/local/run_scenario.py`, which drives `sim_up.sh`
    ...
    **rosbag2 → MCAP** artifact per run as evidence (`runtime/local/run_scenario.py`).
  ...
  - **`runtime/local/run_gate.py` is the flight gate.** It re-derives pass/fail from the numbers
...
- **Host-side logic, message contracts, parsers, pure functions, build-time
  ...
  workstation. `./runtime/local/run_local_ci.sh` is the accepted substitute; say when a result
...
**Version-lock is the architecture.** Record every SHA/tag/digest in `third_party/versions.lock` before
...
- Every deviation — and every source edit — is recorded in the component's vendoring
  ...
  **`third_party/sources.repos` (vcstool) manifest** over submodules for third-party trees.
