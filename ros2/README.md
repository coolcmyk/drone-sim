...
[`../third_party/sources.repos`](../third_party/sources.repos) and checked out under [`../vendor/`](../vendor/).
...
`runtime/local/sim_up.sh` mounts this tree at `/ros2_ws_src` inside the `sim-ros2` container, copies
...
into `/airsim_root` by `runtime/local/build_airsim_wrapper.sh` — it has to be built in place
