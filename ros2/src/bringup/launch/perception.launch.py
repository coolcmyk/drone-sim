"""Perception bring-up: `airsim_node`, with the two fixes it cannot run correctly without.
...
REQUIRES the Cosys-AirSim wrapper to be built and sourced — it is not part of this workspace:
    ...
    ./runtime/local/sim_up.sh && ./runtime/local/build_airsim_wrapper.sh
...
dead. Two separate defects stack up (both measured on 2026-08-02, `SIM-04`):
...
**The retired Gazebo baseline paid for this exact failure shape once.** The cause there was
Gazebo owning time with no bridge running; here it is a parameter default plus a topic name.
Same symptom, same cost, different root — which is precisely why it is worth encoding in a
launch file rather than leaving in someone's shell history.

`publish_clock` DEFAULTS TO TRUE HERE, unlike upstream
------------------------------------------------------
The simulator owns time, so this graph essentially always wants the clock. The remap below is
unconditional: harmless when nothing publishes, and it means there is no configuration in
which the node publishes a clock that nobody can find.

`use_sim_time` STILL DEFAULTS TO FALSE
--------------------------------------
Enabling it is only safe because this file guarantees a
...
in one tested place, per docs/conventions.md.
"""
...
def generate_launch_description() -> LaunchDescription:
    args = [
        DeclareLaunchArgument(
            ...
            description="AirSim RPC host. 127.0.0.1 because every container in the stack "
                        "shares the renderer's network namespace."),
        DeclareLaunchArgument(
            "publish_clock", default_value="true",
            description="Upstream defaults this to false. The simulator owns time, so this "
                        "graph wants it on."),
        DeclareLaunchArgument(
        ...
        DeclareLaunchArgument(
            ...
            description="GPU LiDAR poll period. 10 Hz matches RotationsPerSecond in "
                        "simulator/unreal/settings.json - polling faster only re-reads a frame."),
        DeclareLaunchArgument(
    ...
    return LaunchDescription(args + [
        LogInfo(msg=["perception: airsim_node, clock remapped to /clock. ",
