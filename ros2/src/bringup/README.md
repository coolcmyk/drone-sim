...
```bash
ros2 launch bringup perception.launch.py                 # after ./runtime/local/sim_up.sh
...
workspace — build it with `./runtime/local/build_airsim_wrapper.sh` and source
...
`simulator/unreal/settings.json`. **The frame rate is capped by this file, not by the hardware**:
