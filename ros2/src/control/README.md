...
With the stack up (`./runtime/local/sim_up.sh`):
...
`state_timeout_s` (60.0), `waypoints_enu`, `result_path`. `runtime/local/run_scenario.py` drives
exactly this command with the scenario's values, and `runtime/local/run_park_tour.sh` does the same
for `park_tour`.
...
That cost a full day before it was understood. `runtime/local/sim_up.sh` verifies and repairs the
origin before declaring a stack usable, and `runtime/local/run_gate.py` scores such a run **VOID**
