...
SVC=${SVC:-sim-ros2}
ROOT=/airsim_root
PATCHDIR=simulator/unreal/patches/cosys-airsim
...
for pf in "${patches[@]}"; do
  # SKIP THE UNREAL-SIDE PATCHES. simulator/unreal/patches/cosys-airsim/ holds deviations for BOTH halves of
  # Cosys-AirSim: the ROS 2 wrapper (this build) and the Unreal plugin (applied by
  # runtime/local/convert_world.sh to a world's injected plugin copy). This build root contains
