...
So the rule lives here now, once. Python callers import it; shell callers invoke it as a CLI:
    ...
    python3 runtime/local/collision_witness.py start
    python3 runtime/local/collision_witness.py stop --save out/run-collisions.json
...
def start() -> bool:
    ...
    try:
        ...
        for f in ("watch_collisions.py", "airsim_rpc_client.py"):
            subprocess.run(["docker", "cp", str(REPO / "runtime" / "local" / f), f"{SVC}:/tmp/{f}"],
