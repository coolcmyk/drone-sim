...
SETTINGS="$REPO/simulator/unreal/settings.json"
...
measure() {  # measure <label> <lumen> <forceupdate>
  ...
  bash "$REPO/runtime/local/sim_up.sh" >"$OUT/$label.up.log" 2>&1 \
    || { log "  bring-up failed, see $OUT/$label.up.log"; return 1; }
  bash "$REPO/runtime/local/build_airsim_wrapper.sh" >"$OUT/$label.build.log" 2>&1 \
  ...
  docker cp "$REPO/runtime/local/verify_sensors.py" sim-ros2:/tmp/verify.py >/dev/null
