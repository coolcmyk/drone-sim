...
#   ./runtime/local/sim_up.sh --spawn 50,-30,-10       (metres, NED: Z NEGATIVE is UP)
#   SPAWN=50,-30,-10,315 ./runtime/local/sim_up.sh     (fourth component is yaw, degrees)
SPAWN=${SPAWN:-}
...
usage: sim_up.sh [--world PATH.uproject] [--settings PATH.json]
  ...
  --spawn X,Y,Z[,YAW]   vehicle spawn in metres, NED. Z NEGATIVE is UP: for 10 m of
                        ...
                        simulator/unreal/settings.json; the committed file is never modified.
...
BASE_SETTINGS="${SETTINGS_FILE:-$REPO/simulator/unreal/settings.json}"
...
if [ -n "$SPAWN" ] || [ "$BASE_SETTINGS" != "$REPO/simulator/unreal/settings.json" ]; then
  RUN_SETTINGS="$REPO/simulator/unreal/.settings.run.json"
  cp "$BASE_SETTINGS" "$RUN_SETTINGS"
  chmod 644 "$RUN_SETTINGS"
  SETTINGS="$RUN_SETTINGS"
  [ "$BASE_SETTINGS" != "$REPO/simulator/unreal/settings.json" ] && log "settings: $BASE_SETTINGS"
fi
if [ -n "$SPAWN" ]; then
  ...
  # Gitignored as simulator/unreal/.settings.run.json.
  ...
  python3 "$REPO/runtime/local/apply_spawn.py" "${args[@]}" || die "spawn rejected; not starting"
fi
...
wait_for_settled_vehicle() {
  ...
  docker cp "$REPO/runtime/local/airsim_rpc_client.py" "$SIM:/tmp/airsim_rpc_client.py" >/dev/null
...
/Script/AirSim.AirSimGameMode:
  ...
  Fix it by injecting properly:  runtime/local/inject_airsim.py <your.uproject> --map <YourMap> --force
...
budget; the derived-data cache persists in the sim-ddc volume, so the next run is much faster:
    SIM_LINK_TIMEOUT=3600 ./runtime/local/sim_up.sh ..."
  fi
...
verify_origin() {
  docker cp "$REPO/runtime/local/check_ekf_origin.py" sim-ros2:/tmp/check_ekf_origin.py >/dev/null
  docker exec sim-ros2 bash -lc '
    set +u; source /opt/ros/jazzy/setup.bash
    source /ros2_ws/install/setup.bash 2>/dev/null; source /ros2_ws_src/install/setup.bash 2>/dev/null
...
docker run -d --name sim-ros2 "${ROS2_NS[@]}" ${DS_ENV[@]+"${DS_ENV[@]}"} \
  -v "$REPO/ros2:/ros2_ws_src" -v "$REPO/out:/out" \
