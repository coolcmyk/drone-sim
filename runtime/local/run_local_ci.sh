...
#
#   ./runtime/local/run_local_ci.sh              # fast checks only (~30 s)
#   ./runtime/local/run_local_ci.sh --gate       # + the 10-seed flight gate (~19 min)
#   ./runtime/local/run_local_ci.sh --gate --seeds 3
#
...
RUN_GATE=0
SEEDS=10
SCENARIO=config/scenarios/square-10m.yaml
while [ $# -gt 0 ]; do
  case "$1" in
...
step "image refs declared"     python3 runtime/local/check_image_refs.py
step "worklog renders"         python3 runtime/local/check_worklog_renders.py
step "third_party/sources.repos matches lock"     python3 runtime/local/check_repos_manifest.py
step "no AI attribution"       ./runtime/local/check_attribution.sh
step "third_party/versions.lock conflicts" python3 runtime/local/check_versions_conflicts.py
...
if [ "$RUN_GATE" = "1" ]; then
  ...
  if python3 -u ./runtime/local/run_gate.py "$SCENARIO" --seeds "$SEEDS" --outdir out; then
