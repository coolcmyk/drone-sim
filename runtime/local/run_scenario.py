...
"""Run one seeded scenario against the simulator and emit a structured result.
    ...
    ./runtime/local/run_scenario.py config/scenarios/square-10m.yaml --seed 3
...
`ros2/src/evaluation/` later; driving containers does not.
...
SIM_UP = REPO / "runtime" / "local" / "sim_up.sh"
...
# The GPU-LiDAR drop counter lives in ONE place (runtime/local/lidar_drops.py) because three callers
...
def run_flight(scenario: dict, seed: int) -> dict:
    ...
    if os.environ.get("SIM_NO_VIDEO", "") not in ("1", "true", "yes"):
        ...
        for f in ("watch_video.py", "airsim_rpc_client.py"):
            sh(["docker", "cp", str(REPO / "runtime" / "local" / f), f"{ROS2}:/tmp/{f}"], timeout=60)
    ...
    if a.no_restart:
        if not stack_is_up():
            sys.exit(f"--no-restart given but container {ROS2} is not running; "
                     "bring the stack up with ./runtime/local/sim_up.sh first")
