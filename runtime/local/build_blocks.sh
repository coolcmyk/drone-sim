...
#
#   ./runtime/local/build_blocks.sh            # apply Unreal-side patches, rebuild if anything changed
#   ./runtime/local/build_blocks.sh --force    # rebuild even when the patches were already in place
#
# WHY THIS EXISTS
# ---------------
# Quickstart 0.2 builds the plugin with upstream's own `build.sh`, from PRISTINE vendor source.
# Nothing then applies `simulator/unreal/patches/cosys-airsim/*.patch` to it. `convert_world.sh` does that job for
...
# simulator/unreal/patches/cosys-airsim/ touching Unreal/ is added or changed.
#
...
[ -f "$ROOT/Blocks.uproject" ] || die "no Blocks.uproject at $ROOT
       Run quickstart step 0.2 first: vcs import vendor < third_party/sources.repos, then upstream's build.sh."
...
applied=0; already=0
for p in "$REPO"/simulator/unreal/patches/cosys-airsim/*.patch; do
  [ -e "$p" ] || continue
...
[ "$applied" -gt 0 ] || [ "$already" -gt 0 ] \
  || warn "no Unreal-side patches found under simulator/unreal/patches/cosys-airsim/ -- nothing to apply"
...
log "fly it with ./runtime/local/sim_up.sh"
