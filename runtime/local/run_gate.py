...
"""The simulator's flight gate: success rate over N seeded runs (SIM-07).
    ...
    ./runtime/local/run_gate.py config/scenarios/square-10m.yaml            # 10 seeds, restarting each
    ./runtime/local/run_gate.py config/scenarios/square-10m.yaml --reuse    # faster, weaker (see below)
...
_spec = importlib.util.spec_from_file_location("run_scenario", REPO / "runtime" / "local" / "run_scenario.py")
rs = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(rs)

# The witness plumbing lives in ONE place now (runtime/local/collision_witness.py). It used to exist
...
def _run_origin_check() -> int:
    ...
    """
    checker = REPO / "runtime" / "local" / "check_ekf_origin.py"
    ...
    while True:
        ...
        if rc == ORIGIN_STALE:
            return ("EKF origin is STALE -- it disagrees with GPS, and an EKF origin is set "
                    ...
                    "(runtime/local/sim_up.sh does this). This run is VOID, not a failure.")
...
def main() -> int:
    ...
    if summary["met"]:
        ...
        print("                 at all. Fix the bring-up ordering (runtime/local/sim_up.sh)")
