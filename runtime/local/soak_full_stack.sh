...
#
# WHY THIS ARM EXISTS. The isolated capture soak (runtime/local/soak_capture.py) survived 6000
...
#
# The pass condition changed too. With simulator/unreal/patches/cosys-airsim/0006 an empty readback is survivable,
...
bash "$REPO/runtime/local/sim_up.sh" > "$OUT/fullstack.up.log" 2>&1 || { log "bring-up FAILED"; exit 1; }
bash "$REPO/runtime/local/build_airsim_wrapper.sh" > "$OUT/fullstack.build.log" 2>&1 || { log "wrapper build FAILED"; exit 1; }
...
docker run -d --name soak-rpc \
  ...
  -v "$REPO/runtime/local:/scripts:ro" -v "$OUT:/out" \
  drone-sim/airsim-client:1 python3 /runtime/local/_soak_capture.py \
...
if [ "${LIDAR_ARM:-1}" = "1" ]; then
  ...
  for f in _soak_gpulidar.py airsim_rpc_client.py; do
    docker cp "$REPO/runtime/local/$f" sim-ros2:/tmp/ >/dev/null \
...
while :; do
  ...
  # runtime/local/lidar_drops.py owns the marker string and the container name; see its header.
  DROPS=$(python3 "$REPO/runtime/local/lidar_drops.py" 2>/dev/null || echo -1)
