...
#   ./runtime/local/run_park_tour.sh --world /path/CityPark.uproject --spawn 50,-30,-10
#
...
bash "$REPO/runtime/local/sim_up.sh" "${up_args[@]}" > "$RUN/stack.log" 2>&1 \
...
bash "$REPO/runtime/local/build_airsim_wrapper.sh" >> "$RUN/stack.log" 2>&1 \
...
WITNESS_OK=1
python3 "$REPO/runtime/local/collision_witness.py" start >/dev/null || WITNESS_OK=0
[ "$WITNESS_OK" -eq 1 ] || \
...
COLL_OUT=$(python3 "$REPO/runtime/local/collision_witness.py" stop --save "$RUN/collisions.json")
COLL_RC=$?
...
# COLL_RC and COLLIDED were set above by runtime/local/collision_witness.py, which owns the scoring.
...
# Counted by runtime/local/lidar_drops.py, which OWNS the marker string and the container name --
