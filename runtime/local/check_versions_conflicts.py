#!/usr/bin/env python3
"""Assert every `status: CONFLICT` in third_party/versions.lock carries a summary.
...
Shared by tier-1 CI and runtime/local/run_local_ci.sh so a local pass and a CI pass mean the same
thing.
"""
import re
import sys
...
def main() -> int:
    text = (REPO / "third_party/versions.lock").read_text(encoding="utf-8")
    bad = []
    total = 0
    ...
    if bad:
        print(f"third_party/versions.lock: CONFLICT without a summary at line(s) {bad}", file=sys.stderr)
        return 1
    ...
    if stale:
        for line_no, claimed in stale:
            print(f"third_party/versions.lock:{line_no}: `why_not_{claimed}_yet` in a block whose status "
