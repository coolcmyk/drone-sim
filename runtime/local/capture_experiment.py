...
def build_settings(overrides, width, height, fov):
    ...
    Deliberately NOT derived from simulator/unreal/settings.json -- that file carries PX4, the LiDAR
...
def client_run(script_args, timeout, outdir=None):
    ...
    cmd = [
        ...
        "-v", f"{REPO / "runtime" / "local"}:/scripts:ro",
        "-v", f"{outdir}:/out",
...
def run_variant(name, overrides, args, outdir):
    ...
    try:
        ...
        png = f"/out/{name}.png"
        r = client_run([
            "/runtime/local/_capture_client.py", "--out", png,
