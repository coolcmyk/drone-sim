...
#   ./runtime/local/convert_world.sh /path/to/Your.uproject --map /Game/Map/Small_City_LVL --force
#   ./runtime/local/convert_world.sh /path/to/Your.uproject --no-build     # inject + patch only
#
...
#      there was no collision geometry anywhere. simulator/unreal/patches/cosys-airsim/0005 fixes it.
#
...
python3 "$REPO/runtime/local/inject_airsim.py" "${inject_args[@]}" \
...
[ "$applied" -gt 0 ] || [ "$skipped" -gt 0 ] || warn "no Unreal-side patches found under simulator/unreal/patches/cosys-airsim/"
...
if [ "$DO_BUILD" -eq 0 ]; then
  ...
  if [ "$rc" != "0" ]; then
    ...
    die "build FAILED (exit $rc). The errors above are the real ones; UBT's own summary is not.
       ...
       AirSim plugin, they belong in a patch under simulator/unreal/patches/cosys-airsim/ rather than a build flag."
  fi
  docker rm -f "$cname" >/dev/null 2>&1 || true
...
cat <<EOF
  ...
  Fly it:
      ./runtime/local/sim_up.sh --world "$UPROJECT" --spawn 0,0,-50
