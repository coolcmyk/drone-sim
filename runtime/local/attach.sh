...
#
#   ./runtime/local/attach.sh                                  # interactive shell, ROS 2 sourced
#   ./runtime/local/attach.sh ros2 topic list
#   ./runtime/local/attach.sh --image my/autonomy:latest ros2 run my_pkg my_node
#   ./runtime/local/attach.sh --volume "$PWD/my_ws:/my_ws" bash -lc 'cd /my_ws && colcon build'
#
# WHAT THIS IS FOR
#
...
docker inspect "$SIM" >/dev/null 2>&1 \
  || die "$SIM is not running -- bring the simulator up first:  ./runtime/local/sim_up.sh"
...
docker inspect sim-ros2 >/dev/null 2>&1 \
  || die "sim-ros2 is not running -- the uXRCE-DDS bridge lives there, so there is no
       /fmu/* graph to attach to. Re-run ./runtime/local/sim_up.sh"
