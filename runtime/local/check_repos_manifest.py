#!/usr/bin/env python3
"""Assert `third_party/sources.repos` agrees with `third_party/versions.lock`.

`third_party/sources.repos` says so itself: "Versions here MUST agree with third_party/versions.lock. third_party/versions.lock is the
...
On 2026-07-31 it did not, in two places, and one of them mattered:
  ...
  Micro-XRCE-DDS-Agent   third_party/sources.repos v2.4.2   third_party/versions.lock v2.4.3
  px4_ros_com            third_party/sources.repos main     third_party/versions.lock release/1.16

v2.4.2 is recorded in third_party/versions.lock as GENUINELY UNBUILDABLE - eProsima deleted the Fast-DDS
branch its superbuild pins, so it fails with `fatal: invalid reference: 2.12.x`. Anyone
running `vcs import vendor < third_party/sources.repos` on a fresh machine checked out the version that cannot
...
A `third_party/sources.repos` entry passes if its `version` matches the lock entry's sha, tag, OR branch - so
either pinning style is accepted, while real drift is caught. Entries commented out in
`third_party/sources.repos` are not checked; entries with no mapping below are reported so the map cannot
silently rot.
...
# third_party/sources.repos entry -> the path through third_party/versions.lock that owns its version
MAP: dict[str, tuple[str, ...]] = {
    ...
    # Retired trees stay commented out in third_party/sources.repos; the mapping is kept so that
...
class _NoDuplicateKeys(yaml.SafeLoader):
    ...
    That is not academic here. On 2026-08-01 an edit to third_party/versions.lock replaced a coupling's
...
def main() -> int:
    ...
    # value.
    try:
        lock = yaml.load((REPO / "third_party/versions.lock").read_text(encoding="utf-8"), _NoDuplicateKeys)
    except yaml.YAMLError as exc:
        print(f"third_party/versions.lock has a duplicate key:\n  {exc}", file=sys.stderr)
        return 1
    try:
        repos_doc = yaml.load((REPO / "third_party/sources.repos").read_text(encoding="utf-8"), _NoDuplicateKeys)
    except yaml.YAMLError as exc:
        print(f"third_party/sources.repos has a duplicate key:\n  {exc}", file=sys.stderr)
        return 1

    repos = (repos_doc or {}).get("repositories") or {}

    # FLOOR GUARD. Without this the whole check passes vacuously: an empty third_party/sources.repos, a
    ...
    if missing:
        problems.append(
            f"third_party/sources.repos is missing entries that must be active at this phase: "
    ...
    for c in lock.get("couplings") or []:
        cid = c.get("id")
        if cid in seen_ids:
            problems.append(f"third_party/versions.lock: duplicate coupling id {cid!r}")
        seen_ids.add(cid)
        for field in ("assert", "why", "severity"):
            if field not in c:
                problems.append(f"third_party/versions.lock: coupling {cid!r} is missing {field!r}")
    ...
    for name, entry in sorted(repos.items()):
        ...
        if path is None:
            problems.append(
                f"{name}: active in third_party/sources.repos but not mapped in {Path(__file__).name} "
        ...
        if not isinstance(node, dict):
            problems.append(f"{name}: third_party/versions.lock has no entry at {'.'.join(path)}")
            continue
        ...
        # A resolved SHA is a hard requirement, not one option among several. third_party/sources.repos's own
        # header: "When a `sha: TODO-verify` in third_party/versions.lock is resolved, pin the SHA here
        # too - a tag can move, a SHA cannot." The first version of this checker accepted
        # sha OR tag OR branch, which let px4_msgs sit on a live branch while third_party/versions.lock
        # had a SHA - the exact drift it was written to catch.
        if sha and sha != "TODO-verify":
            if version != sha:
                problems.append(
                    f"{name}: third_party/sources.repos version={version!r} but third_party/versions.lock "
            ...
            problems.append(
                f"{name}: third_party/versions.lock {'.'.join(path)} pins no sha/tag/branch to check against"
            )
        elif version == "TODO-verify" or version in ("", "None"):
            problems.append(
                f"{name}: third_party/sources.repos version={version!r} is a sentinel, not a pin - "
    ...
    if problems:
        print("third_party/sources.repos / third_party/versions.lock drift check FAILED:", file=sys.stderr)
        for p in problems:
            print(f"  - {p}", file=sys.stderr)
        print(
            "\nversions.lock is the authority; third_party/sources.repos is the mechanism. A disagreement means "
            "`vcs import vendor < third_party/sources.repos` reconstructs a tree that is not the one recorded as "
    ...
    print(f"{len(repos)} active third_party/sources.repos entries, all agree with third_party/versions.lock")
    return 0
