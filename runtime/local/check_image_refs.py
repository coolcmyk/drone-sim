...
"""Every `drone-sim/...` image reference names an image this repo actually builds.
    ...
    python3 runtime/local/check_image_refs.py
...
---------------------
`third_party/versions.lock`, per its own contract: the `images:` section lists every image the repo
...
LOCK = REPO / "third_party/versions.lock"
...
EXCLUDE_FILES = {"runtime/local/check_image_refs.py"}   # this file quotes tags to explain them

BINARY_SUFFIXES = (".png", ".jpg", ".jpeg", ".zip", ".mp4", ".pdf")


def built_images() -> set[str]:
    """Tags declared under `images:` in third_party/versions.lock.

    Parsed with a line scanner rather than a YAML load on purpose: third_party/versions.lock is read by
...
def main() -> int:
    ...
    if not declared:
        print("FAIL: no images: section found in third_party/versions.lock — is the path right?")
        return 1
    ...
    if problems:
        ...
        for p in problems:
            print(f"  {p}")
        print(f"\n  declared in third_party/versions.lock: {', '.join(sorted(declared))}")
        print("\n  Either fix the reference, or add the image to third_party/versions.lock images: "
              "with its dockerfile and role.")
        return 1

    print(f"OK: {len(seen)} distinct image reference(s), all declared in third_party/versions.lock")
