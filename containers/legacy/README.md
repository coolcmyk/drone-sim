# `containers/legacy/` — previous multi-image local workflow

This directory preserves the pre-Fern local Docker workflow while contributors migrate to the
supported `containers/stack/Dockerfile` image. It is not the Runpod image path.

For a local multi-container run, [`../../runtime/local/sim_up.sh`](../../runtime/local/sim_up.sh)
drives raw `docker run`; the services share the renderer network and IPC namespaces.
...
```bash
./runtime/local/sim_up.sh                                  # the whole stack, ~80 s
./runtime/local/sim_up.sh --world /path/to/Your.uproject   # your own Unreal world
```
...
`third_party/versions.lock` is the authority on that list: `runtime/local/check_image_refs.py` fails tier-1 CI
...
- **`ros2.Dockerfile` no longer installs `ros-gz-bridge`.** It has no consumer: `/clock`
  ...
  `ros2/src/bringup/launch/perception.launch.py` (`/airsim_node/clock` → `/clock`). The
...
| `sim-ros2` | `drone-sim/ros2:v1.16.0` | **Jetson Orin NX** | the uXRCE-DDS bridge, this repo's reference nodes (`interfaces`/`control`/`bringup`) and the AirSim wrapper. **User code does not go here** — it attaches from outside with `runtime/local/attach.sh` |
