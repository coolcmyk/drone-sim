...
#
# WHY THIS IS SEPARATE FROM containers/legacy/video.Dockerfile
# ------------------------------------------------------------
# containers/legacy/video.Dockerfile adds ffmpeg, to re-encode renders after a flight. None of that is
...
#
# The pin still lives in third_party/versions.lock; the difference is that the build enforces it rather
...
# Keep these in step with the qgroundcontrol entry in third_party/versions.lock.
