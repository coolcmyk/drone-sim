...
#
# This image is also the BASE of containers/legacy/qgc.Dockerfile and containers/legacy/ros2.Dockerfile, and it
# supplies both the PX4 SITL container and the uXRCE-DDS agent container in
# runtime/local/sim_up.sh. Retagging it is never a one-file change.
#
...
#
# Build:  docker build -f containers/legacy/px4.Dockerfile -t drone-sim/px4:v1.16.0 .
...
# containers/legacy/ros2.Dockerfile with the code that uses them. Declaring them in both files would give a
# pin two sources of truth, and nothing cross-checks Dockerfile ARGs against third_party/versions.lock -- a
...
COPY runtime/local/px4-entrypoint.sh /usr/local/bin/entrypoint.sh
